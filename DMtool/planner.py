...
def load_templates():
    try:
        with open(r'DMtool\planner.py\prompt_templates.yaml', 'r', encoding='utf-8') as file:
            data = yaml.safe_load(file)
            return {name: info['template'] for name, info in data['prompt_templates'].items()}
    except:
        return {}  # if no templates found return a empty dict
